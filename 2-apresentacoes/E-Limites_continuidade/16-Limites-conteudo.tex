...
%------------------------------------------------------------------------------%
\begin{frame}[t]{Exemplo \theexemplo}
  \onslide<+->{Ao calcular o limite \((x, y) \to (0, 0)\), assumimos \((x, y) \neq (0, 0)\)}
  \begin{align*}
    \onslide<+->{\lim_{(x, y)\to(0, 0)} \frac{x^2-xy}{\,\sqrt{x}-\sqrt{y}\,}}
    \onslide<+->{& = \lim_{(x, y)\to(0, 0)}
                     \frac{\left(x^2-xy\right) \emph{\left(\sqrt{x}+\sqrt{y}\right)}}
                          {\left(\sqrt{x}-\sqrt{y}\right)\emph{\left(\sqrt{x}+\sqrt{y}\right)}} \\[1mm]}
    \onslide<+->{& = \lim_{(x, y)\to(0, 0)}
                     \frac{x\left(\emph{x-y}\right)\left(\sqrt{x}+\sqrt{y}\right)}{\emph{x-y}} \\[1mm]}
    ...
    \onslide<+->{& = 0\left(\sqrt{0}+\sqrt{0}\right)}
    \onslide<+->{\;=\; 0}
  \end{align*}
\end{frame}
