%------------------------------------------------------------------------------%
\begin{question}
  Calcule
  \begin{gather*}
    a = \big[(3 + 2i) + (4 - 5i)\big](1 + i)                      \\[7mm]
    b = \big[ (2 + 3i) (1 - i) \big] \big[ (1 + 2i) (2 - i) \big] \\[7mm]
    c = \frac{3 + 4i}{2 - i}
  \end{gather*}
\end{question}

%------------------------------------------------------------------------------%
\begin{resolution}{Solução}
  \begin{align*}
  \onslide<+->{a & = \big[(3 + 2i) + (4 - 5i)\big](1 + i) \\}
  \onslide<+->{  & = [7 - 3i\,] (1 + i)                     \\}
  \onslide<+->{  & = 7 + 7i -3i -3i^2                     \\}
  \onslide<+->{  & = 7 + 7i -3i +3                        \\}
  \onslide<+->{  & = 10 + 4i                                }
  \end{align*}
\end{resolution}

%------------------------------------------------------------------------------%
\begin{resolution}{Solução}
  \begin{align*}
    \onslide<+->{b & = \big[ (2 + 3i) (1 - i) \big]
                       \big[ (1 + 2i) (2 - i) \big]               \\}
    \onslide<+->{  & = \left[ 2 - 2i + 3i - 3i^2 \right]
                       \left[ 2 -  i + 4i - 2i^2 \right]          \\}
    \onslide<+->{  & = [ 2 - 2i + 3i + 3 ] [ 2 -i + 4i + 2 ]      \\}
    \onslide<+->{  & = ( 5 + i ) ( 4 + 3i )                       \\}
    \onslide<+->{  & = 20 + 15i + 4i +3i^2                        \\}
    \onslide<+->{  & = 20 + 15i + 4i -3                           \\}
    \onslide<+->{  & = 17 + 19i                                     }
  \end{align*}
\end{resolution}

%------------------------------------------------------------------------------%
\begin{resolution}{Solução}
  \begin{align*}
    \onslide<+->{c & = \frac{3 + 4i}{2 - i}                   \\}
    \onslide<+->{  & = \frac{(3 + 4i)(2 + i)}{(2 - i)(2 + i)} \\}
    \onslide<+->{  & = \frac{6 + 3i + 8i -4}{4 + 2i -2i +1}   \\}
    \onslide<+->{  & = \frac{2 + 11i}{5}                      \\}
    \onslide<+->{  & = \frac{2}{5} + \frac{11}{5}i              }
  \end{align*}
\end{resolution}

%------------------------------------------------------------------------------%
