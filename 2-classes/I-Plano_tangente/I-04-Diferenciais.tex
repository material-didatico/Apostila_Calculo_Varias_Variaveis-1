...
\begin{frame}{Variação em uma Dimensão}
  Em uma dimensão podemos estimar a variação do valor da função em um ponto próximo à $a$
  usando a derivada em $a$
...
\begin{frame}{Variação em Várias Dimensões}
  ...
  \[
    df
    = \left(D_u f\evalat{(a, b)}{}\right) ds
    \pause
    = \left(\nabla f\evalat{(a, b)}{} \cdot u\right) ds
  \]
  \pause
  \[
    ds = \sqrt{(x - a)^2 + (y-b)^2}
