...
\column{0.45\linewidth}
  ...
  \begin{align*}
    ...
    \onslide<+->{x + x^4                 & = 0 \\}
    \onslide<+->{x\left(1+ x^3\right)    & = 0}
  \end{align*}
\column{0.55\linewidth}
  \begin{align*}
    \onslide<+->{x=0 \quad\text{ ou }\quad}
    \onslide<+->{1+ x^3 & = 0            \\}
    \onslide<+->{x^3    & = -1           \\}
    \onslide<+->{x  \;  & = \sqrt[3]{-1} \\}
    \onslide<+->{x  \;  & = -1}
  \end{align*}
  \onslide<+->{As soluções dessa equação são\\ \emph{$x=0$} ou \emph{$x=-1$}}
\end{columns}
\end{resolution}
...
\begin{resolution}{d -- Classificando os Pontos}
  ...
  \pause
  Portanto, o ponto $(0, 0)$ é um \emph{ponto de sela}
\end{resolution}
...
\begin{resolution}{d -- Classificando os Pontos}
  ...
  Como \(f_{xx}(-1, -1) = -6 < 0\)
  \pause
  o ponto é um ponto de \emph{máximo local}
\end{resolution}
